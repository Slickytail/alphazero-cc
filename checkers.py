...
BoardSpace = Tuple[int, int]
Move = Tuple[BoardSpace, BoardSpace]
...
class CheckersGame():
    ...
    def clone(self):
        newgame = CheckersGame(self._board.copy(), self.player_turn)
        newgame.winner = self.winner
        return newgame

    def move(self, start: BoardSpace, end: BoardSpace, **kwargs):
        ...
        if (FULL_BOARD[end] > 0
            ...
            and self.opposite(FULL_BOARD[end]) in self.colors):
                ...
                self.winner = self.colors.index(CheckersGame.opposite(FULL_BOARD[end]))
        else:
            # Otherwise it's the next player's turn
            self.player_turn = (self.player_turn + 1) % self.n_players

    def is_legal(self, start: BoardSpace, end: BoardSpace) -> bool:
    ...
    def get_legal(self, player) -> Generator[Move, None, None]:
    ...
    def paths(self, start: BoardSpace) -> Generator[BoardSpace, None, None]:
